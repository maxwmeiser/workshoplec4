...
std::string Join(std::vector<std::string> pieces, std::string glue){
	...
	for(int i = 0; i < size; i++){
		//edge case: last addition doesnt need glue phrase after
		if(i == size){
			toReturn += pieces.at(i);
			break;
		}

		toReturn += pieces.at(i)+ glue;
	}
	return toReturn;
};
...
int RemoveTwos(int original){
	//if can divide by 2, do so. else, return
	while(true){
		if(original % 2 == 0){
			original = original / 2;
		} else {
...
std::vector<bool> EvenMask(std::vector<int> ints){
	std::vector<bool> toReturn;
	int vecSize = ints.size();
	//parse vector backwards, push_back each value
